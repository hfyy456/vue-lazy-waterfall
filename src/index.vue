...
export default {
  ...
  created() {
    ...
    this.totalWidth = 1200 / Math.ceil(this.size / this.column)
    ...
    setPosition(column, list) {
      ...
      for (let i = 0; i < row; i++) {
        heights.push([
          ...
          this.totalWidth * i + 10 * i
        ])
      }
      let index = this.getMinHeight(heights)
      for (let i = row; i < list.length; i++) {
        list[i].position = 'absolute'
        list[i].left = heights[index][1] + 10
        list[i].top = heights[index][0] + 10
        heights[index][0] +=
          list[i].height * (this.totalWidth / list[i].width) + 10
