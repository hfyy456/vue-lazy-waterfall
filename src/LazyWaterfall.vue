<template>
  <div style="width: 100%">
    <div class="lazywaterfall-container">
      <img
        v-for="(img, i) of imgList"
        :style="{
          width: `${totalWidth}px`,
          height: `${img.height * (totalWidth / img.width)}px`,
          left: `${img.left}px`,
    ...
    <div class="none" v-show="none">已经到底了</div>
  </div>
</template>
...
export default {
  ...
  beforeDestroy() {
    let self = this
    window.removeEventListener('resize', function reset() {
      self.totalWidth =
  ...
  methods: {
    ...
    async setPosition(list) {
      ...
      for (let i = 0; i < this.row; i++) {
        ;[list[i].width, list[i].height] = await this.getSize(list[i].url)
      ...
      let maxHeight = Math.max(
        ...
        document.getElementsByClassName('lazywaterfall-container')[0].clientHeight
      )
      let currClient =
      ...
      for (let i = this.row; i < list.length; i++) {
        if (maxHeight > heights[index][0] && currClient > heights[index][0]) {
          ...
          list[i].src = list[i].url
          heights[index][0] +=
            list[i].height * (this.totalWidth / list[i].width) + this.padding
          index = this.getMinHeight(heights)
        }
      }
      document.getElementsByClassName(
        'lazywaterfall-container'
